...
plt.savefig("figures/low_lampvariation.png", bbox_inches='tight')
plt.clf()
...
plt.ylabel("Verticle Position on pdu/cm")
plt.colorbar(bar)
plt.savefig("figures/low_lampvariation_err.png", bbox_inches='tight')
...
flat_array = np.ndarray.flatten(upper_grid/np.max(upper_grid))
flat_error = np.ndarray.flatten(upper_error_grid)
...
for i in range(8):
    ...
    cum_intensity_error.append(cum_total_error/40)
...
try:
    ...
    p0 = [max(cum_intensity), np.median(cum_x_axis),1,min(cum_intensity)] # this is an mandatory initial guess

    dn_parmas, pcov = curve_fit(sigmoid, cum_x_axis, cum_intensity,p0, method='lm')
...
plt.savefig("figures/verticle_intensity_change.png", bbox_inches='tight')
plt.clf()
...
plt.savefig("figures/low_lamphistogram.png")
plt.clf()
